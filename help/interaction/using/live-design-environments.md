...
Par défaut, Interaction est livré avec un environnement pré-paramétré pour cibler le tableau des destinataires (offres identifiées). Si vous souhaitez cibler une autre table (table des visiteurs et visiteuses pour les offres anonymes ou table des personnes destinataires spécifique), vous devez effectuer les configurations suivantes :
