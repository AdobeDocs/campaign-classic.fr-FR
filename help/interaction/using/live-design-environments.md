...
Par défaut, Interaction est livré avec un environnement pré-paramétré pour cibler le tableau des destinataires (offres identifiées). Si vous souhaitez cibler une autre table (table des visiteurs pour les offres anonymes ou table de destinataires spécifique), vous devez effectuer le paramétrage suivant :
