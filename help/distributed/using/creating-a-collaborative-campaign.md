...
Lʼentité centrale crée les campagnes collaboratives à partir des modèles de campagne **Marketing distribué**.Reportez-vous à [cette page](about-distributed-marketing.md#collaborative-campaign).
...
Le processus de configuration d’une base de données de campagne collaborative est similaire à celui d’un modèle de campagne locale.Les spécificités des différents types de campagnes collaboratives sont présentées ci-dessous.
...
Sélectionnez l’interface web **Par formulaire**.Ce type d’interface vous permet de créer des champs de personnalisation qui seront utilisés par les entités locales lors de la commande d’une campagne.Pour plus d&#39;informations, consultez la section [Création d’une campagne locale (par formulaire)](examples.md#creating-a-local-campaign--by-form-).
...
Lorsque la commande d’une **Campagne collaborative (par campagne)** est approuvée par l’entité centrale, une campagne enfant est créée pour l’entité locale.La mise à disposition de la campagne permet à l’entité locale de modifier les éléments suivants :

* le workflow de l&#39;opération,
* les règles de typologie,
* et les champs de personnalisation.

L’entité locale exécute la campagne enfant.L’entité centrale exécute la campagne parente.
...
Le workflow de la campagne doit intégrer l’activité de type **Validation en local**.Les paramètres de l’activité sont les suivants :
...
La répartition des données de type **Répartition des entités locales** doit être créée.Le modèle de répartition des données vous permet de limiter le nombre d’enregistrements à partir d’une liste de valeurs de groupement.Dans **[!UICONTROL Ressources > Gestion de campagnes > Répartition de données]**, cliquez sur l&#39;icône **[!UICONTROL Nouveau]** pour créer une **[!UICONTROL Répartition de données]**. Pour plus d&#39;informations sur la répartition de données, voir le guide [Workflows](../../workflow/using/using-the-local-approval-activity.md#step-1--creating-the-data-distribution-template-).
...
>[!NOTE]
>
>L’option **[!UICONTROL Date butoir de personnalisation]** permet à l’entité centrale de choisir une date butoir avant laquelle les entités locales doivent lui faire parvenir des documents (tableurs, images) qui pourraient lui être utiles pour la configuration de la campagne.Il ne s’agit pas d’une option obligatoire.Le contournement de cette date n’affecte pas la mise en œuvre de la campagne.
...
En mode automatique, l’entité locale est pré-abonnée pour la campagne.Elle peut se désinscrire de la campagne ou en modifier le paramétrage sans qu’aucune validation par l’entité centrale ne soit nécessaire.
...
Ce message permet également aux entités locales de visualiser la description renseignée par l’opérateur central qui a créé le kit, ainsi que les documents associés à la campagne.Ils n’appartiennent pas à la campagne elle-même, bien qu’ils fournissent des informations supplémentaires à son sujet.
