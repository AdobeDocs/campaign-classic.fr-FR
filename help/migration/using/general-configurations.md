...
Dans la version 6.02, le mode « à plusieurs fuseaux horaires » n’était disponible que pour les moteurs de base de données PostgreSQL.Il est désormais proposé quel que soit le type de moteur de base de données utilisé.Nous vous recommandons vivement de transformer votre base en une base « à plusieurs fuseaux horaires ».

Pour utiliser le mode TIMESTAMP WITH TIMEZONE, vous devez également ajouter l’option **-userTimestamptz:1** à la ligne de commande de postupgrade.

>[!IMPORTANT]
>
>Si le paramètre **-usetimestamptz:1** est utilisé avec un moteur de base de données incompatible, votre base de données sera corrompue et vous devrez restaurer une sauvegarde de votre base de données et réexécuter la commande ci-dessus.
...
Si vous obtenez l’erreur **ORA 01805** lors du postupgrade, cela signifie qu’il existe une désynchronisation des fichiers de fuseaux horaires Oracle entre le serveur d’applications et le serveur de base de données.Pour les resynchroniser, effectuez les étapes suivantes :
...
* Le tracking ne fonctionne plus si le mot de passe est vide.Si c’est le cas, un message d’erreur vous en informera et vous invitera à le reconfigurer.
* Les mots de passe des utilisateurs et utilisatrices ne sont plus stockés dans le schéma **xtk:sessionInfo**.
...
Dans la version 5, le paramètre **sessiontoken** fonctionnait à la fois côté client (liste des écrans de type vue d’ensemble, éditeur de lien, etc.)et côté serveur (applications web, rapports, jsp, jssp, etc.).Dans v7, il ne fonctionne que côté serveur. Si vous souhaitez revenir aux fonctionnalités complète telles que dans la version 5, vous devez modifier les liens à l&#39;aide de ce paramètre et les transmettre via la page de connexion :
...
La syntaxe **[!UICONTROL myObject.@attribute]** n’est désormais valide que pour les objets XML.Cette syntaxe peut être utilisée pour personnaliser les diffusions et la gestion de contenu. Si vous avez utilisé ce type de syntaxe sur un objet non XML, les fonctions de personnalisation ne fonctionneront plus.

Pour tous les autres types d&#39;objet, la syntaxe est désormais **[!UICONTROL myObject`[`&quot;attribute&quot;`]`]**. Par exemple, un objet non XML qui utilisait la syntaxe suivante : **[!UICONTROL employee.@sn]**, doit désormais utiliser la syntaxe suivante : **[!UICONTROL employee`[`&quot;sn&quot;`]`]**.
...
Lorsqu’un attribut « @expr » est utilisé, le SQLData peut être présent.Une recherche peut être effectuée sur les termes suivants : « SQLData », « aliasSqlTable », « sql ».
...
* Nouvelle syntaxe :
  ...
  >[!NOTE]
  >
  >Les jointures sont réalisées automatiquement pour les fonctions d’agrégat.Il n’est plus nécessaire de préciser la condition WHERE O0.iOperationId=iOperationId.
  >
...
* **[!UICONTROL Conserver la version actuelle]** : a pour conséquence de refuser la mise à jour de la version.
  ...
  >[!IMPORTANT]
  >
  >Si vous sélectionnez ce mode de résolution, vous risquez de perdre des correctifs inclus dans la nouvelle version.Il est donc vivement recommandé de ne pas utiliser cette option ou de la réserver à des opérateurs et opératrices experts.
...
>[!IMPORTANT]
>
>Si certaines diffusions utilisant des offres configurées doivent être envoyées après la migration, vous devez supprimer et recréer toutes ces diffusions dans la version 7.Si vous ne pouvez pas le faire, un « mode de compatibilité » est proposé.Ce mode n’est pas recommandé, car vous ne bénéficierez pas de toutes les nouvelles fonctionnalités d’Interaction v7.Il s’agit d’un mode de transition qui vous permet d’effectuer des campagnes en cours avant la migration réelle vers la version 6.1.Pour plus d’informations concernant ce mode, veuillez nous contacter.
...
>[!NOTE]
>
>L’utilisation de ce script ne vous permet pas de bénéficier des options « gestion de contenu » et « fonctions de rendu ».Pour bénéficier de ces fonctions, vous devez repenser le catalogue d’offres, en particulier le contenu des offres et la configuration des emplacements.
...
Tous les rapports standard utilisent actuellement le moteur de rendu v6.x.Si vous aviez ajouté JavaScript dans ces rapports, certains éléments peuvent ne plus fonctionner.En effet, l’ancienne version de JavaScript n’est pas compatible avec le moteur de rendu v6.x.Vous devez donc vérifier le code JavaScript et l’adapter ultérieurement.Vous devez tester chaque rapport, en particulier la fonction d’export.
...
Testez vos applications web dans le moteur de rendu v6.x, puis publiez-les.Désactivez ensuite ces deux options.
