...
# Configuration de événements pour une implémentation personnalisée {#events}

Certaines parties de cette configuration sont un développement personnalisé et nécessitent les éléments suivants :
...
Les [!DNL trigger] données sont transmises à la fonction JS au format XML.
...
Le contenu est défini au format JSON en Adobe Analytics pour chaque déclencheur.
Par exemple, dans un déclencheur LogoUpload_uploading_Visits :

* **[!UICONTROL eVar01]** peut contenir l&#39;identifiant Shopper au format String, qui est utilisé pour la conciliation avec les destinataires Adobe Campaign. <br>Il doit être réconcilié pour trouver l&#39;identifiant de nouvel acheteur, qui est la clé principale.

* **[!UICONTROL timeGMT]** peut contenir l&#39;heure du déclencheur côté Adobe Analytics au format Epoque UTC (secondes depuis le 01/01/1970 UTC).
...
### Événement de traitement{#order-events}
...
Les logs tels que logInfo() sont dirigés vers le log [!DNL pipelined]. Des erreurs telles que logError() sont écrites dans le log [!DNL pipelined] et entraînent le placement de l’événement dans une file d’attente de reprise. Dans ce cas, vous devez vérifier le journal en pipeline.
Les messages en erreur sont repris plusieurs fois dans la durée définie dans les options [!DNL pipelined].

À des fins de débogage et de surveillance, les données de déclenchement complètes sont écrites dans le tableau de déclenchement dans le champ &quot;données&quot; au format XML. Une autre solution consiste à utiliser un logInfo() contenant les données de déclenchement dans le même but.
...
Les performances de ce code doivent être optimales, car il s’exécute à des fréquences élevées et peut avoir des effets négatifs potentiels pour d’autres activités marketing. Surtout si le traitement de plus d’un million de événements de déclenchement par heure sur le serveur Marketing ou s’il n’est pas correctement réglé.
...
Les événements sont stockés dans une table de base de données. Il est utilisé par les campagnes marketing pour cible des clients et enrichit les courriels à l’aide de déclencheurs.
...
La réconciliation est le processus de mise en correspondance du client d&#39;Adobe Analytics avec la base de données Adobe Campaign. Par exemple, les critères de correspondance peuvent être shopper_id.
...
Il est possible d’exécuter la requête de réconciliation pour chaque déclencheur dans JavaScript. Les performances sont ainsi supérieures et les résultats sont plus rapides. Cela peut être nécessaire pour des cas pratiques spécifiques lorsqu’une grande réactivité est requise.

Il peut s’avérer difficile à mettre en oeuvre si aucun index n’est défini sur shopper_id. Si les critères se trouvent sur un serveur de base de données distinct de celui du serveur marketing, il utilise un lien de base de données, qui offre de faibles performances.
