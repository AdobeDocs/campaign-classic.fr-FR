...
1. Installez le package **[!UICONTROL Integration with the Adobe Experience Cloud]** standard. L’installation d’un package d’intégration est identique à l’installation d’un package standard, qui est détaillée dans la section [Importation](../../platform/using/working-with-data-packages.md#importing-packages) de package. Vous pouvez ainsi accéder aux ressources partagées via Digital Asset Manager.
...
1. In **[!UICONTROL Administration > Platform > Options]**, configure the server and organization (Tenant) options for Adobe Target:
   ...
   * **[!UICONTROL TNT_EdgeServer]** : Serveur Adobe Target utilisé pour l’intégration. Cette option est déjà sélectionnée par défaut. This value corresponds to the Adobe Target **[!UICONTROL Domain Server]**, followed by the value **/m2**. Par exemple : **tt.omtrdc.net/m2**.
