...
ACS Connector relie Adobe Campaign v7 à Adobe Campaign Standard. Il s&#39;agit d&#39;une fonctionnalité intégrée de Campaign v7 qui reproduit automatiquement les données vers Campaign Standard, associant le meilleur des deux applications. Campaign v7 contient des outils avancés pour gérer la base de données marketing principale. La réplication des données depuis Campaign v7 permet à Campaign Standard d&#39;exploiter les données enrichies dans un environnement convivial.
...
Par défaut, la réplication périodique du connecteur ACS a lieu toutes les 15 minutes. La durée de la réplication périodique peut être ajustée en fonction de vos besoins. Contactez votre consultant si des modifications s’avèrent nécessaires.

La réplication des données pour les destinataires, les abonnements, les services et les landing pages est incrémentielle, ce qui signifie que seuls les nouveaux destinataires et les modifications apportées aux destinataires existants sont répliqués de Campaign v7 vers Campaign Standard. Cependant, la réplication pour une audience se produit dans une seule instance. Vous pouvez créer une audience dans Campaign v7, puis la répliquer une fois dans Campaign Standard. La réplication est immédiate et ne peut pas être configurée pour des mises à jour régulières. Pour plus d’informations, voir [Synchronisation des audiences](../../integrations/using/synchronizing-audiences.md).

>[!NOTE]
>
>Soyez patient avec la réplication initiale d&#39;une base de données volumineuse, car elle peut prendre plusieurs heures. Toutefois, les réplications suivantes sont incrémentielles et beaucoup plus rapides.
...
* **Profils en lecture seule pour Campaign Standard** : les profils répliqués sont en lecture seule dans Campaign Standard. Cependant, vous pouvez modifier les destinataires dans Campaign v7 et les modifications sont automatiquement mises à jour dans Campaign Standard par ACS Connector.
* **Profils créés dans Campaign Standard** : ACS Connector réplique les données des destinataires dans un sens, de Campaign v7 vers Campaign Standard. Par conséquent, les profils issus de Campaign Standard ne sont pas répliqués vers Campaign v7.
* **Données de destinataire de base pour Campaign Standard** : ACS Connector réplique les données des destinataires adaptées à Campaign Standard. Elle comprend le nom, l’adresse, l’adresse e-mail, le numéro de téléphone mobile, le numéro de téléphone personnel et d’autres coordonnées utiles. Si d’autres champs de destinataire et tables de ciblage personnalisées disponibles dans Campaign v7 sont essentiels à votre workflow, veuillez contacter votre consultant.
* **Import des profils mis en quarantaine** : des listes de profils qui ne souhaitent pas être contactés peuvent être importées dans Campaign v7 ou Campaign Standard en tant que profils mis en quarantaine. Le statut des profils est inclus dans la synchronisation des quarantaines entre les applications et ils ne seront pas utilisés dans les diffusions.
* **Désabonnement à un service dans Campaign Standard** : le choix de se désabonner d’une diffusion n’est pas synchronisé de Campaign Standard vers Campaign v7. Cependant, vous pouvez configurer une diffusion Campaign Standard pour rediriger son lien de désinscription vers Campaign v7. Le profil d&#39;un destinataire qui clique sur le lien de désinscription est mis à jour dans Campaign v7 et les données sont répliquées vers Campaign Standard. Consultez la section [Modification du lien de désinscription](../../integrations/using/synchronizing-profiles.md#changing-the-unsubscription-link).
...
Il existe deux types de mises en œuvre d’pour ACS Connector. Toutes les deux sont toujours effectuées par l’équipe Consulting d’Adobe Campaign.
...
>
>N’essayez en aucun cas d’effectuer ces mises en œuvre par vous-même. Elle est strictement réservée aux consultants Adobe Campaign.
...
Pour utiliser la fonctionnalité, le package **[!UICONTROL ACS Connector]** doit être installé. Cette opération est toujours effectuée par l’administrateur technique ou le consultant Adobe.
...
>[!IMPORTANT]
>
>N’essayez jamais de modifier ces workflows. Ils ne doivent jamais être en erreur ou en pause. Si cela se produit, contactez votre consultant Adobe Campaign.
...
Les workflows de réplication suivants sont disponibles en tant que modèles « prêts à l’emploi ». Ils doivent être mis en œuvre par votre consultant Adobe Campaign.
...
Si vous disposez de champs supplémentaires ou de tables personnalisées (table de transactions, par exemple), ils ne seront pas répliqués par défaut. Une configuration avancée doit être effectuée. Voir [Mise en œuvre avancée](#advanced-implementation).

Vous trouverez ci-dessous la liste des champs des destinataires répliqués avec l’implémentation de base. Voici les champs d’usine :
...
Les droits sont gérés différemment dans Campaign v7 et Campaign Standard. Dans Campaign v7, la gestion des droits est basée sur les dossiers, alors que dans Campaign Standard, elle repose sur l’accès aux entités (entités organisationnelles/géographiques). Un utilisateur Campaign Standard appartient au groupe de sécurité qui contient le contexte de restriction. Par conséquent, le système de droits de Campaign v7 doit être converti pour correspondre à celui de Campaign Standard. Il existe plusieurs façons d’effectuer la conversion des droits. Vous trouverez ci-dessous un exemple d’implémentation.

1. Sous **[!UICONTROL Administration > ACS Connector > Gestion des droits > Groupes de sécurité]**, utilisez le bouton **[!UICONTROL Synchroniser]** pour récupérer tous les groupes de sécurité de Campaign Standard. Les groupes Campaign Standard prêts à l’emploi sont exclus.
...
>[!IMPORTANT]
>
>Ces informations ne peuvent être utilisées qu’à titre indicatif. Contactez votre consultant Adobe Campaign pour la mise en œuvre.

L’implémentation avancée ajoute des workflows de réplication personnalisés, en fonction des besoins du client. Voici quelques exemples :
...
Avec l’implémentation de base, les champs de destinataire prêts à l’emploi sont répliqués. Si vous souhaitez répliquer les champs personnalisés que vous avez ajoutés au schéma des destinataires, vous devez les identifier.
...
Avec l’implémentation de base, la table des destinataires prête à l’emploi est répliquée. Si vous avez ajouté des tables de destinataires personnalisées, voici comment les identifier.
