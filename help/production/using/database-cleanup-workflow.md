...
Le workflow **[!UICONTROL Nettoyage de la base]** , accessible à partir du nœud **[!UICONTROL Administration > Exploitation > Workflows techniques]** permet de supprimer les données obsolètes afin d&#39;éviter une croissance exponentielle de la base de données. Le workflow est déclenché automatiquement sans intervention de l’utilisateur.
...
Par défaut, le workflow **[!UICONTROL Nettoyage de la base de données]** est configuré pour démarrer tous les jours à 4 heures du matin. Le planificateur vous permet de modifier la fréquence de déclenchement du workflow. Les fréquences disponibles sont les suivantes :
...
La première tâche exécutée par le workflow **[!UICONTROL Nettoyage de la base de données]** supprime tous les groupes dont l’attribut **deleteStatus != 0** est **NmsGroup**. Les enregistrements liés à ces groupes et qui existent dans d’autres tables sont également supprimés.
...
1. Chaque liste comporte plusieurs liens vers d’autres tables. Tous ces liens sont supprimés en bloc à l’aide de la requête suivante :
...
1. Le workflow inclut ensuite la liste des diffusions ayant récemment expiré et utilisant le mid-sourcing. Les diffusions pour lesquelles aucun log de diffusion n&#39;a encore été récupéré via le serveur de mid-sourcing sont exclues.
...
1. Tous les fragments (**deliveryParts**) des diffusions obsolètes sont supprimés et tous les fragments obsolètes des diffusions de notification en cours sont supprimés. Une suppression en masse est utilisée pour ces deux tâches.
...
1. Les diffusions purgées sont ensuite identifiées, afin d’éviter d’avoir à les retraiter ultérieurement. La requête suivante est exécutée :
...
1. Les tables utilisées par les workflows en cours sont alors exclues. Pour cela, la liste des diffusions en cours est récupérée à l&#39;aide de la requête suivante :
...
1. Toutes les tables utilisées par une instance de workflow en attente sont alors exclues. La liste des workflows actifs est récupérée à l&#39;aide de la requête suivante :

   ```sql
   SELECT iWorkflowId FROM XtkWorkflow WHERE iWorkflowId<>0 AND iState<>20
   ```

1. Chaque identifiant de workflow est ensuite récupéré pour trouver le nom des tables utilisées par les workflows en cours. Ces noms sont exclus de la liste des tables récupérées précédemment.
...
Cette tâche purge, dans la table **NmsSubscription**, les abonnements que l’utilisateur a supprimés à l’aide d’une suppression en masse. La requête suivante est utilisée :
...
1. Le workflow **Nettoyage de la base** purge ensuite les données obsolètes des tables récupérées précédemment. La requête suivante est utilisée :
...
1. Enfin, le workflow vérifie si la table **NmsProviderMsgId** existe. Si tel est le cas, toutes les données obsolètes sont supprimées à l&#39;aide de la requête suivante :
