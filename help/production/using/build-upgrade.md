...
description: Découvrez les principales étapes pour réaliser un upgrade de build
...
L’upgrade de build doit être effectué avec précaution. Les effets de l’upgrade doivent être entièrement étudiés au préalable et la procédure doit être suivie à la lettre. Pour garantir la réussite d’un upgrade, veillez à ce que seuls les utilisateurs experts effectuent la procédure décrite ci-dessous. De plus, il est vivement recommandé de contacter l’[Assistance clientèle d’Adobe](https://helpx.adobe.com/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html) avant de commencer un upgrade.
...
Une fois que le système prêt, un upgrade dure **au moins** 2 heures.
...
1. Téléchargez le dernier build. [En savoir plus sur le Centre de téléchargement](https://docs.adobe.com/content/help/fr-FR/experience-cloud/software-distribution/home.html).
...
Seuls les clients **on-premise** doivent suivre ces procédures. Pour les clients hébergés, elle est gérée par l’équipe d’hébergement. Pour mettre à jour Adobe Campaign vers un nouveau build, les étapes suivantes doivent être effectuées.
...
1. Exécutez le script de cautérisation **nms:freezeInstance.js** sur l&#39;environnement-cible avant sa mise en route. Cela permettra l’arrêt de tous les processus qui interagissent avec l’extérieur : logs, tracking, diffusions, workflows de campagnes, etc.

   ```
   nlserverjavacsriptnms:freezeInstance.js–instance:<dev> -arg:run
   ```

1. Vérifier la cautérisation, comme suit :
...
1. Vérifiez à l’aide du Gestionnaire de tâches de Windows que tous les processus sont arrêtés.
...
1. Sélectionnez le mode d&#39;installation : choisissez **Mise à jour** ou **Réparation**.
...
### Mettre à jour les consoles clientes
...
Lorsque la messagerie transactionnelle (Message Center) est activée sur votre instance Campaign, vous devez effectuer les étapes supplémentaires suivantes pour réaliser l’upgrade :
...
1. Vérifiez que la version a été mise à jour en utilisant un lien de test. Par exemple :
...
>[!NOTE]
>
>Le serveur de mid-sourcing doit toujours exécuter la même version (ou une version plus récente) que pour les serveurs marketing.
...
Le fichier **postupgrade_ServerVersionNumber_TimeOfPostupgrade.log** contient le résultat de la synchronisation. Il est disponible par défaut dans le répertoire suivant :**nstallationDirectory/var/instanceName/postupgrade**. Les erreurs et les avertissements sont indiqués par les attributs d’erreur et d’avertissement.

### Analyser les conflits

**Comment un conflit est-il détecté ?**

Il est possible de détecter des conflits dans le fichier postupgrade.log situé sur le serveur en question ou dans l&#39;interface du client Campaign (Administration > Paramétrage > Gestion des packages > Édition des conflits).
...
1. Fusion complexe : lorsque du code est requis. Des compétences en développement sont nécessaires. Voir [Fusions complexes](#complex-merges).
...
1. Le code intégré est stocké dans des fichiers XML dans le dossier datakit. Recherchez le fichier XML correspondant à l’objet en conflit, par exemple : installationDirectory\datakit\nms\fra\form\recipient.xml
1. Récupérez la version d’origine : depuis le [Centre de téléchargement](https://docs.adobe.com/content/help/fr-FR/experience-cloud/software-distribution/home.html) ou une autre installation non mise à niveau du produit.
1. Récupérez la nouvelle version : depuis le [Centre de téléchargement](https://docs.adobe.com/content/help/fr-FR/experience-cloud/software-distribution/home.html) ou les fichiers installés par le client.
...
1. Dans la partie inférieure de la fenêtre, faites une recherche sur la chaîne **_conflict_string_** pour localiser les entités en conflit. L’entité installée avec la nouvelle version contient l’argument new, l’entité correspondant à la version précédente contient l’argument custom.
...
1. Décider de ce qu’il faut faire avec la modification.
...
1. Testez les non-régressions de la personnalisation
1. Vérifiez le fonctionnement des modifications
1. Demandez au client de réaliser des tests d’acceptation des utilisateurs
...
* [Options d’aide et de support pour Campaign Classic](https://helpx.adobe.com/fr/campaign/kb/ac-support.html#acc-support-req)
