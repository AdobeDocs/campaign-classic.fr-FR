...
L’exploration des données de la base est proposée à travers le module **Marketing Analytics**.Permet d’analyser et de mesurer des données, de calculer des statistiques, de simplifier et d’optimiser la création et le calcul de rapports.En outre, le module Marketing Analytics vous permet de créer des rapports et de créer des populations cibles.Une fois ces derniers identifiés, ils sont stockés dans des listes qui peuvent être utilisées dans Adobe Campaign (ciblage, segmentation, etc.).

Les cubes sont utilisés pour la génération de certains rapports intégrés, notamment les rapports de diffusion (suivi des diffusions, clics, ouvertures, etc.).Les rapports basés sur les cubes ne peuvent être utilisés que comme norme pour les volumes de données inférieurs à 5 millions de lignes de faits.
...
Découvrez comment créer et utiliser des modèles de diffusion dans la [documentation d’Adobe Campaign v8](https://experienceleague.adobe.com/docs/campaign/campaign-v8/analytics/reports/cubes/gs-cubes.html?lang=fr){target="_blank"}.
