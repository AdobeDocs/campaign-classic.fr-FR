---
title: Utilisation de la table de  Adobe Campaign Classic
description: Découvrez comment utiliser le tableau de prêt à l’emploi dans  Adobe Campaign Classic lors de la conception de votre modèle de données.
...
Ce présente  recommandations clés lors de la conception de votre modèle de données .

Pour une meilleure compréhension des tableaux intégrés Campaign et de leur interaction, reportez-vous à la section [Campaign Classic modèle](../../configuration/using/about-data-model.md) de données.

Lisez cette [documentation](../../configuration/using/about-schema-reference.md) pour commencer à utiliser Campaign . Apprenez à configurer les  afin d&#39;étendre le modèle de données conceptuelles de la base de données  dans ce [](../../configuration/using/about-schema-edition.md).

## Présentation {#overview}

 système Adobe Campaign est extrêmement flexible et peut être étendu au-delà de la mise en oeuvre initiale. Cependant, même si les possibilités sont infinies, il est essentiel de prendre des décisions judicieuses et de construire des fondations solides pour à la conception de votre modèle de données.
...
## Architecture du modèle de données {#data-model-architecture}
 ...
 Adobe Campaign Standard est un puissant système dede  de qui peut vous aider à aligner vos stratégies en ligne et hors ligne pour créer des expériences client personnalisées.

### Approche axée sur le client {#customer-centric-approach}

Bien que la plupart des de messagerie communiquent avec les clients par le biais d’une approche centrée sur les ,  s’appuie sur une base de données relationnelle afin de tirer parti d’un plus large de clients et de leurs attributs.

Cette approche axée sur le client est présentée dans le graphique ci-dessous. La table **** en gris représente la table client principale autour de laquelle tout est construit :
...
Le modèle de données par défaut  Adobe Campaign est présenté dans ce [](../../configuration/using/data-model-description.md).
...
Quelles données doivent être envoyées à  Adobe Campaign ? Il est essentiel de déterminer les données requises pour votre  de  marketing.
...
* Attribut utilisé pour les processus **de** ( calcul de , par exemple)
* Attribut utilisé pour la **personnalisation**

Si vous ne tombez pas dans l&#39;un de ces attributs, vous n&#39;aurez probablement pas besoin de cet attribut dans  Adobe Campaign.
...
* Il est acceptable d’avoir un champ plus court dans  Adobe Campaign que dans le système source si vous êtes certain que la taille du système source a été surestimée et ne serait pas atteinte. Cela peut signifier une chaîne plus courte ou un entier plus petit dans  Adobe Campaign.
...
Pour les instances hybrides et sur site, le  (Federated Data Access, fonctionnalité facultative permettant d’accéder aux données externes) répond à la nécessité d’ajouter un champ &quot;à la volée&quot; lors d’un processus de campagne. Vous n’avez pas besoin de tout importer si vous avez des . Pour plus d’informations, voir [A propos des Federated Data Access](../../platform/using/about-fda.md).

### Choix des clés {#choice-of-keys}

Outre le code **automatique** défini par défaut dans la plupart des tableaux, vous devez envisager d’ajouter des clés logiques ou professionnelles (numéro de compte, numéro de client, etc.). Il peut être utilisé ultérieurement pour les importations/rapprochement ou les  de. For more on this, see [Identifiers](#identifiers).
...
## Identificateurs {#identifiers}
 ...
 ressources Adobe Campaign comporte trois identifiants et il est possible d’ajouter un identifiant supplémentaire.
...
* Evitez d’utiliser des opérations telles que &quot;CONTAINS&quot; dans les  de. Si vous savez ce qui est attendu et souhaitez être filtré, appliquez la même condition avec un opérateur &quot;ÉGAL À&quot; ou d’autres opérateurs de filtre spécifiques.
* Evitez de vous joindre aux champs non indexés lors de la création de données dans les  de.
...
* Assurez-vous qu&#39;il y a un horaire pour tous les  de la quotidienne  et respectez le calendrier.
* Si un ou quelques processus quotidiens échouent et s’il est obligatoire de les exécuter le même jour, assurez-vous qu’il n’y a pas de processus conflictuels en cours d’exécution lorsque le processus manuel est lancé, car cela pourrait affecter les performances du système.
...
* Une chaîne courte reste acceptable. Au cas où des tableaux de référence sont déjà en place dans un système externe, réutiliser la même méthode facilitera l’intégration des données avec  Adobe Campaign.
...
* Il est bon d’avoir tous les champs essentiels dans un tableau car cela facilite la création de  de pour les utilisateurs. Il est parfois également judicieux de de certains champs entre des tableaux, si cela permet d’éviter une jointure.
* Certaines fonctionnalités intégrées ne pourront pas faire référence à des relations de type &quot;un à plusieurs&quot;, par exemple,  formule de pondération  et  de.
...
* Lors de l’utilisation de tableaux de personnalisés supplémentaires, veillez à disposer d’une table de journal dédiée pour chaque mappage de  de.
...
* Réduisez autant que possible la profondeur de rétention du journal. Si vous avez besoin d’un historique plus détaillé, vous pouvez   le calcul et/ou gérer des tables de journaux personnalisées pour stocker un historique plus volumineux.
