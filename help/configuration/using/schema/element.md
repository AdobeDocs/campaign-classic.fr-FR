...
* **applicableIf (string)** : condition pour l&#39;application de l&#39;index. Cet attribut reçoit une expression XTK.
* **autopk (boolean)** : si cette option est activée (autopk=« true »), une clé unique est automatiquement définie. Cette option ne peut être utilisée que sur l&#39;élément principal du schéma. Attention, Adobe Campaign ne garantit que l&#39;unicité de la clé générée. Il n’est pas garanti que les valeurs de clé soient consécutives et incrémentielles.
* **dataPolicy (string)** : permet de préciser des contraintes de validation sur les valeurs permises dans le champ SQL. Les valeurs de cet attribut sont les suivantes :
...
* **expr (string)** : cet attribut définit un champ calculé pour lequel aucune définition n&#39;est stockée dans la table. Il reçoit un Xpath ou une expression XTK (string).
* **externalJoin (boolean)**: jointure externe dans un élément de type &quot;link&quot;.
* **feature (string)**: définit un champ de caractéristiques. Les champs de caractéristiques sont utilisés pour étendre les données d&#39;une table existante, mais avec un stockage déporté dans une table annexe. Les valeurs acceptées sont les suivantes :
...
* **featureDate (boolean)** : attribut associé au champ de caractéristiques « @feature ». Si sa valeur est « true », il vous permet de savoir quand la valeur a été mise à jour pour la dernière fois.
...
* **folderModel (string)** : définit le type de dossier qui permet le stockage des entités. Cet attribut n’est défini que si « @folderLink » est présent.
* **folderProcess (chaîne)** : définit le lien où les instances de modèle d’entité sont stockées. Cet attribut n’est défini que si « @folderLink » est présent.
...
* **img (string)**: reçoit le chemin d&#39;une image associée a l&#39;élément. La valeur de cet attribut est de type « espace de noms :image nom ». Par exemple : img=« cus:myImage.jpg ». Physiquement l&#39;image doit être importée sur le serveur applicatif.
...
* **length (string)** : max. nombre de caractères autorisés pour une valeur du champ SQL de type « string ».
...
* **name (MNTOKEN)** : nom interne de l&#39;élément correspondant au nom de la table. La valeur de l&#39;attribut « @name » doit être courte, de préférence en anglais, et respecter les contraintes de nommage liées au XML.
...
* **pkSequence (string)** : reçoit le nom de la séquence à utiliser pour le calcul d&#39;une clé auto-incrémentale. Cet attribut ne peut être utilisé que si une clé auto-incrémentale est définie sur l’élément racine du schéma.
...
* **ref (string)** : cet attribut définit une référence à un élément >element> partagé par plusieurs schémas (factorisation des définitions). La définition n’est pas copiée dans le schéma actuel.
* **obligatoire (booléen)** : si cet attribut est activé (@required=« true »), le champ est mis en surbrillance dans l’interface. Le libellé du champ est rouge dans les formulaires.
...
* **revIntegrity (string)** : cet attribut définit l&#39;intégrité sur le schéma cible. Les mêmes valeurs que l&#39;attribut « @integrity » sont autorisées. Par défaut, Adobe Campaign donne la valeur « normal » à cet attribut.
...
* **sqlname (string)** : nom du champ lors de la création de la table. Si « @sqlname » n&#39;est pas spécifié, la valeur de l&#39;attribut « @name » est utilisée par défaut. Lors de l’écriture du schéma dans la table, des préfixes sont automatiquement ajoutés en fonction du type de champ.
* **sqltable (string)**: pour l&#39;élément principal du schéma, cet attribut surcharge le nom de la table SQL générée par défaut. Si « @sqltable » n’est pas spécifié, le nom par défaut est structuré comme suit : espace de noms (majuscule en première lettre) suivi de la valeur du SrcSchema « @name ».
...
* **target (MNTOKEN)** : reçoit le nom du schéma cible lors de la création d&#39;un lien entre les tables. Cet attribut est uniquement actif pour les éléments de type « link ».
...
* **userEnum (string)** : reçoit le nom interne d&#39;une énumération « open ». Les valeurs d’énumération peuvent être définies par l’utilisateur dans l’interface .
* **xml (boolean)** : si cette option est activée, toutes les valeurs définies dans l&#39;élément sont stockées en XML dans un champ « mData » de type TEXT. Cela signifie qu’il n’y aura aucun filtrage ni tri sur ces champs.
