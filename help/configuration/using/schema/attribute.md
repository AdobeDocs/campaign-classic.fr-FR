...
* **dataPolicy (string)** : permet de préciser des contraintes de validation sur les valeurs permises dans le champ de type SQL ou XML. Les valeurs pour cet attribut sont les suivantes :
...
* **enum (string)** : reçoit le nom de l’énumération associé au champ. L’énumération peut être insérée dans le même schéma ou dans un schéma distant.
* **expr (string)** : définit une expression de précalcul de champ. Cet attribut reçoit un Xpath ou une expression XTK.
...
* **label (string)** : libellé associé au champ, le plus souvent destiné à l’utilisateur ou à l’utilisatrice dans l’interface. Il permet d’éviter les contraintes de dénomination.
* **length (string)** : nombre maximal de caractères autorisés pour une valeur du champ SQL de type « chaîne ». Si l’attribut « @length » n’est pas précisé, Adobe Campaign crée automatiquement un champ de 255 caractères.
...
* **notNull (boolean)** : permet de redéfinir le comportement d’Adobe Campaign concernant la gestion des enregistrements NULL dans la base de données. Par défaut, les champs numériques ne sont pas nuls et les champs de type chaîne et date peuvent être nuls.
...
* **required (boolean)** : si cet attribut est activé (@required=&quot;true&quot;), le champ est mis en avant dans l’interface. Le libellé du champ est affiché en rouge dans les formulaires.
...
* **sqlname (string)** : nom du champ lors de la création de la table. Si @sqlname n’est pas renseigné, la valeur de l’attribut « @name » est utilisée par défaut. Lors de l’écriture du schéma dans la base de données, des préfixes sont automatiquement ajoutés en fonction du type du champ.
...
* **type (MNTOKEN)**: type du champ.
  ...
  Les types de champs sont génériques. En fonction du type de base de donnée installé, Adobe Campaign transforme le type défini en valeur propre à la base de données installée lors de la mise à jour de la structure.
  ...
  * uuid
  ...
  Si le champ est de type STRING et que le nom du champ n’est pas renseigné par la présence de l’attribut « @sqlname », alors le nom du champ dans la base de données sera automatiquement précédé du caractère « s ». Ce fonctionnement sera similaire avec les champs de type INTEGER (i), DOUBLE (d) et DATES (ts).
...
* **xml (boolean)** : si cette option est activée, les valeurs du champ n’ont pas de champ SQL associé. Adobe Campaign crée un champ « mData » de type Texte pour le stockage des enregistrements. Cela signifie qu’il n’y a aucun filtrage ni tri sur ces champs.
