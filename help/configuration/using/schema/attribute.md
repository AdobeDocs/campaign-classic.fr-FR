...
* **autoIncrement (boolean)** : si cette option est activée, le champ devient un compteur. Vous pouvez ainsi incrémenter une valeur (principalement des identifiants). (usage externe)
...
* **dataPolicy (string)** : permet de préciser des contraintes de validation sur les valeurs permises dans le champ de type SQL ou XML. Les valeurs de cet attribut sont les suivantes :
...
* **expr (chaîne)** : définit une expression de précalcul de champ. Cet attribut reçoit un Xpath ou une expression XTK.
...
* **label (chaîne)** : libellé associé au champ, principalement destiné à l&#39;utilisateur dans l&#39;interface. Il permet d’éviter les contraintes de dénomination.
* **length (string)** : max. nombre de caractères pour une valeur du champ SQL de type « string ». Si l&#39;attribut « @length » n&#39;est pas renseigné, Adobe Campaign crée automatiquement un champ de 255 caractères.
...
* **name (MNTOKEN)** : nom de l&#39;attribut qui correspondra au nom du champ dans la table. La valeur de l’attribut « @name » doit être courte, de préférence en anglais, et respecter les contraintes de dénomination XML.
...
* **notNull (boolean)** : permet de redéfinir le comportement d&#39;Adobe Campaign concernant la gestion des enregistrements NULL en base de données. Par défaut, les champs numériques ne sont pas nuls et les champs de type chaîne et date peuvent être nuls.
...
* **sqlname (string)** : du champ lors de la création de la table. Si @sqlname n&#39;est pas renseigné, la valeur de l&#39;attribut « @name » est utilisée par défaut. Lorsque le schéma est écrit dans la base de données, des préfixes sont automatiquement ajoutés en fonction du type de champ.
...
* **type (MNTOKEN)**: type du champ.
  ...
  Les types de champ sont génériques. En fonction du type de base de données installé, Adobe Campaign transforme le type défini en une valeur spécifique à la base de données installée lors de la mise à jour de la structure.
  ...
  Si le champ est de type STRING et que le nom du champ n&#39;est pas renseigné par la présence de l&#39;attribut « @sqlname », le nom du champ en base sera automatiquement précédé d&#39;un &#39;s&#39;. Ce mode de fonctionnement sera similaire avec les champs de type ENTIER (i), DOUBLE (d) et DATES (ts) .

* **userEnum (string)** : reçoit le nom interne d&#39;une énumération « open ». Les valeurs de l’énumération peuvent être définies par l’utilisateur dans l’interface .
...
* **xml (boolean)** : si cette option est activée, les valeurs du champ n&#39;ont pas de champ SQL associé. Adobe Campaign crée un champ « mData » de type Texte pour le stockage des enregistrements. Cela signifie qu’il n’existe aucun filtrage ni tri sur ces champs.
