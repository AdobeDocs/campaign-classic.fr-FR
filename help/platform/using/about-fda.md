...
>[!CAUTION]
>
>L&#39;accès à une base de données externe par l&#39;intermédiaire d&#39;un n&#39;est possible que pour les installations sur site ou hybrides, sauf avec les connecteurs Snowflake. Pour plus d’informations à ce propos, consultez cette [page](https://helpx.adobe.com/fr/campaign/kb/acc-on-prem-vs-hosted.html).

## Principe de fonctionnement {#operating-principle}

L’option  vous permet d’étendre votre modèle de données dans une base de données tierce. Il détecte automatiquement la structure des tables ciblées et utilise les données provenant des sources SQL.
...
1. [Créez le](../../platform/using/creating-data-schema.md) de la base de données externe dans  Adobe Campaign. Cela vous permet de reconnaître la structure des données de la base de données externe.
...
Une fois le de données créé, les données peuvent être traitées dans   Adobe Campaign de. Voir à ce propos [cette section](../../workflow/using/accessing-an-external-database--fda-.md).
