...
1. Accédez au **[!UICONTROL Administration > Platform > External accounts]** dossier de l’arborescence Adobe Campaign.
...
1. The **[!UICONTROL Advanced]** tab lets you configure or modify more technical settings.
   ...
   * **[!UICONTROL Lifespan]**: vous permet de spécifier le délai (en jours_ après lequel les événements Web ont été récupérés dans Adobe Campaign par les processus techniques. Par défaut : 180 jours.
...
>[!NOTE]
>
>Si vous utilisez plusieurs outils de mesure de l’audience, vous pouvez sélectionner **[!UICONTROL Other]** dans la liste **[!UICONTROL Partners]** déroulante lors de la création du compte externe. Vous ne pouvez référencer qu’un compte externe dans les propriétés de remise : vous devrez donc adapter la formule des URL suivies en ajoutant les paramètres attendus par Adobe et tous les autres outils de mesure utilisés.
...
Si d’autres segments ont été configurés dans l’assistant Connecteur de données, vous pouvez les ajouter à Adobe Campaign. Pour ce faire, cliquez sur le **[!UICONTROL Update segment list]** lien et suivez les étapes décrites dans l’assistant de compte externe. Une fois l’opération effectuée, les nouveaux segments s’affichent dans la liste.
...
* **[!UICONTROL Recovering of web events]**: une fois par heure, ce processus télécharge les segments sur le comportement des utilisateurs sur un site donné, les inclut dans la base de données Adobe Campaign et lance le processus de marketing de relance.
* **[!UICONTROL Event purge]**: ce processus vous permet de supprimer tous les événements de la base de données en fonction de la période configurée dans le **[!UICONTROL Lifespan]** champ. Pour plus d’informations à ce sujet, reportez-vous à l’ [étape 2 : Créez le compte externe dans Campaign](#step-2--create-the-external-account-in-campaign).
* **[!UICONTROL Identification of converted contacts]**: du répertoire des visiteurs qui ont effectué un achat à la suite d’une campagne de marketing de relance. Les données collectées par ce processus sont accessibles dans le **[!UICONTROL Re-marketing efficiency]** rapport. Reportez-vous à cette [page](#creating-a-re-marketing-campaign).* **[!UICONTROL Sending of indicators and campaign attributes]**: lets you send email campaign indicators via Adobe Campaign to the Adobe Experience Cloud using Adobe Analytics - Data connector. Ce flux de travaux est déclenché à 4 heures du matin tous les jours et peut prendre 24 heures pour que les données soient envoyées à Analytics.
...
* **Identification des contacts** convertis : du répertoire des visiteurs qui ont effectué un achat à la suite d’une campagne de marketing de relance. Les données collectées par ce processus sont accessibles dans le **[!UICONTROL Re-marketing efficiency]** rapport (reportez-vous à cette [page](../../platform/using/adobe-analytics-data-connector.md#creating-a-re-marketing-campaign)).
...
1. Dans **[!UICONTROL Explorer]**, accédez au **[!UICONTROL Resources > Templates > Delivery templates]** dossier de l’arborescence Adobe Campaign.
...
Le **[!UICONTROL Re-marketing efficiency]** rapport est accessible via la page des rapports globaux. Il vous permet d’afficher le nombre de contacts convertis (c’est-à-dire ayant acheté quelque chose) par rapport au nombre d’abandons de panier après la campagne de remarketing d’Adobe Campaign. Le taux de conversion est calculé par semaine, mois ou depuis le début de la synchronisation entre Adobe Campaign et les outils d’analyse Web.
