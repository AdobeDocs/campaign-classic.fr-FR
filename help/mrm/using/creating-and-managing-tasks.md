...
Adobe Campaign vous permet de créer des tâches et de gérer leur cycle de vie complet directement dans l’application. La mise en œuvre des programmes et des campagnes peut être découpée en tâches qui sont assignées à des opérateurs et opératrices Adobe Campaign ou à des prestataires externes. Ce mode de fonctionnement permet de créer un environnement de collaboration ouvert incluant tous les participantes et participants au programme et les participantes et participants externes.

Les tâches peuvent être créées, visualisées et suivies au niveau de liste des tâches et du tableau de bord d’une campagne. Elles peuvent également être visualisées et suivies au niveau des plannings du plan marketing, des programmes et des campagnes.

Les tâches sont attachées à des campagnes et peuvent avoir des dépendances, c’est-à-dire des tâches associées. Chaque tâche a un statut, une priorité, une charge estimée et des coûts associés.
...
Cette vue filtre automatiquement toutes les tâches afin de n’afficher que les **[!UICONTROL tâches de l’opérateur ou de l’opératrice]**. Vous pouvez également filtrer les tâches à partir des champs de la section supérieure de la fenêtre.
...
Saisissez au minimum le nom de la tâche, puis sélectionnez la campagne à laquelle elle est rattachée. Vous pouvez également spécifier les dates de début et de fin. Ces trois informations sont obligatoires.
...
Vous pouvez spécifier les opérateurs et opératrices qui s’attelleront à la réalisation de la tâche. Ces opérateurs et opératrices ne sont pas autorisés à fermer la tâche. Ils ne peuvent qu’approuver la tâche qui leur est assignée.
...
Cliquez sur **[!UICONTROL OK]**, puis saisissez le taux d’utilisation : celui-ci représente la charge de travail affectée à l’opérateur ou à l’opératrice pendant la durée d’exécution de la tâche. Ce taux est donné à titre indicatif et est exprimé en pourcentage.
...
Pour chaque opérateur ou opératrice, vous pouvez renseigner une charge de travail planifiée et une charge de travail réelle. Ces durées sont également fournies à titre d’information uniquement.
...
Vous pouvez définir un planning de validation pour la tâche dans la section inférieure de la fenêtre de configuration du réviseur ou de la réviseuse. Par défaut, les validant(e)s disposent de trois jours à partir de la date de soumission pour valider une tâche. Il est possible de configurer un rappel qui sera automatiquement envoyé aux opérateurs et opératrices concernés avant la date limite de validation.
...
Cliquez sur **[!UICONTROL Ajouter]** et sélectionnez le document à rattacher à votre tâche. Faites de même pour les ressources marketing.
...
La période de validité d’une tâche est indiquée dans les champs **[!UICONTROL Début]** et **[!UICONTROL Fin]**. La charge planifiée exprime la charge de travail à réaliser durant cette période. Elle est exprimée en jours ou en heures.
...
Le **[!UICONTROL Statut de la progression]** de la tâche, exprimé en pourcentage, est mis à jour automatiquement en fonction du travail réalisé par les différents opérateurs et opératrices impliqués. Il peut être saisi manuellement.
...
Si la date de fin du planning d’exécution de la tâche est atteinte et que la tâche n’est pas terminée, la tâche sera **[!UICONTROL En retard]**. Un message d’avertissement s’affichera également pour alerter les opérateurs et opératrices.
...
Vous pouvez définir les dépenses liées et les revenus prévisionnels pour chaque tâche. Ils seront calculés puis consolidés au niveau de la campagne à laquelle est rattachée la tâche.
...
Par défaut, le budget imputé est celui de la campagne à laquelle est rattachée la tâche. Il est affiché dans les détails de la tâche.
...
Sélectionnez les postes de coûts afférents à l’exécution de la tâche. Pour cela, choisissez le type de coût et ajoutez éventuellement un montant à surcharger.
...
Une tâche est en retard si elle a atteint sa date de fin alors qu’elle n’a pas le statut **[!UICONTROL Terminé]**. Par défaut, personne n’est alerté lorsqu’une tâche est en retard. Vous pouvez configurer la diffusion d’un e-mail de notification : tous les opérateurs et opératrices peuvent recevoir une notification, même s’ils ne sont pas impliqués dans la tâche.

Dans la boîte **[!UICONTROL Ressources]**, choisissez l’opérateur ou l’opératrice dans le champ **[!UICONTROL Affectation]**. Pour avertir plusieurs personnes, sélectionnez un groupe d’opérateurs et d’opératrices.
...
Cependant, si la tâche que vous êtes en train de créer est éloignée dans le temps, il peut être préférable de programmer l’envoi de l’e-mail de notification un peu avant le début de la tâche. Par exemple, si la tâche ne commence que dans un mois, vous pouvez faire en sorte que la personne responsable de la tâche reçoive une notification une semaine avant le début de celle-ci.
...
Vous pouvez créer des tâches directement au niveau d’un programme pour gérer les actions qui relèvent de l’organisation globale de celui-ci et non d’une campagne particulière (par exemple, une réunion pour discuter du thème des prochaines campagnes au sein du programme). La tâche apparaîtra dans le planning du programme.

Pour créer une tâche directement rattachée à un programme :

1. Ouvrez le planning du programme : depuis la page d’accueil, rendez-vous sur **[!UICONTROL Campagnes > Navigation > Autres choix > Programmes]**. Le planning global des programmes s’ouvre dans la section droite de la fenêtre.
...
Lorsque vous créez des tâches depuis une campagne, vous pouvez créer des tâches spécifiques. Le type de tâche est défini dans le modèle sélectionné.
...
Une tâche de création de ressource marketing peut être utilisée pour gérer la création et la publication d’une ressource marketing. Si vous gérez une ressource par le biais d’une tâche et non uniquement de la ressource elle-même, vous pouvez effectuer les opérations suivantes :
...
* L’approbation et la publication de la ressource peuvent être réalisées par le biais de la tâche : si l’option **[!UICONTROL Publier la ressource marketing]** est activée, la ressource est approuvée et publiée automatiquement lorsque la tâche est terminée. Si l’option n’est pas activée, il n’y a pas d’interaction entre la tâche et la ressource : une action effectuée dans l’une n’aura aucun effet sur l’autre.
...
Dans le champ **[!UICONTROL Ressource marketing]**, définissez la ressource que vous voulez gérer via cette tâche. Vous pouvez ainsi :
...
Toutes les tâches regroupées dans une tâche de groupement sont visibles dans le tableau de bord de celle-ci. Cela vous permet de filtrer la liste des tâches afin de n’afficher que les tâches qui vous intéressent.
...
Les tâches de notification vous permettent de planifier les diffusions par e-mail (à un opérateur ou une opératrice, un groupe d’opérateurs et d’opératrices, un prestataire, etc.). Vous pouvez ainsi planifier des rappels, par exemple pour informer une personne qu’une campagne se termine bientôt, ou pour envoyer des documents avant le début d’une campagne afin que les opérateurs et opératrices puissent la préparer. Cela signifie que vous pouvez suivre vos communications au sein de votre campagne ou programme et mieux identifier les actions réalisées.

#### Cycle de vie {#life-cycle}

Les tâches de notification ne nécessitent pas de validation. Leur cycle de vie est donc plus simple que celui d’une tâche standard :
...
* **[!UICONTROL Affecté à]** : l’opérateur ou l’opératrice ou le groupe d’opérateurs et d’opératrices qui recevra l’e-mail. Si vous réaffectez la tâche une fois l’e-mail envoyé, l’e-mail ne sera pas renvoyé à la nouvelle personne affectée à la tâche (pour cela, il faut réinitialiser la tâche et changer sa date de début).
* **Date de début de la tâche** : date à laquelle l’e-mail de notification sera envoyé. Cette date doit impérativement être située dans le futur par rapport au moment où vous enregistrez la tâche.
* **Date de fin de la tâche** : date à laquelle la tâche prend le statut **[!UICONTROL Terminé]**. Par défaut, la date de fin est identique à la date de début. Cependant, donner une durée à la tâche permet, si besoin, de symboliser dans le planning le temps dont dispose l’opérateur ou l’opératrice pour agir.
...
Les statuts des tâches peuvent être affichés dans la carte des tâches. Le statut d’exécution d’une tâche est mis à jour automatiquement en fonction des actions des opérateurs et opératrices.
...
* À sa création, une tâche a le statut **[!UICONTROL Planifié]** si sa date de début se situe dans le futur. Elle garde ce statut tant que sa date de début n’est pas atteinte.
...
* Pour planifier une tâche, saisissez une date de début dans le futur. Vous pouvez alors envoyer une notification initiale aux opérateurs et opératrices Adobe Campaign impliqués dans la réalisation de cette tâche. Voir à ce sujet la section [Cycle de vie complet d&#39;une tâche](#complete-task-life-cycle).

>[!NOTE]
>
>* Le statut de la tâche est mis à jour automatiquement.
>* Même si la période de validité est terminée, une tâche qui n’a pas été close apparaît toujours dans les tâches en cours. Un message d’avertissement est affiché pour alerter les opérateurs et opératrices que la tâche est en retard.
>

### Statut d&#39;avancement d&#39;une tâche {#progress-status-of-a-task}

En plus de son statut d’exécution, une tâche peut être associée à un statut de la progression : **[!UICONTROL En retard]**, **[!UICONTROL À approuver]**, **[!UICONTROL À faire aujourd’hui]** ou **[!UICONTROL À faire cette semaine]**. Ces informations sont automatiquement renseignées en fonction du planning de réalisation de la tâche.
...
1. Le responsable crée la tâche et renseigne les différents champs. Pour plus d&#39;informations, consultez la section [Créer une nouvelle tâche](#creating-a-new-task).
   ...
   Pour envoyer cette première notification, cliquez sur **[!UICONTROL Oui]**. Cette notification les informe de la prochaine tâche, avec le détail de son contenu et le nombre de jours restants pour la réaliser.
...
1. Lorsqu&#39;il estime que la tâche est terminée, le responsable peut alors la clore, à partir de l&#39;email de notification envoyé à la date de début de la tâche, depuis la console ou l&#39;interface web.
   ...
   >[!NOTE]
   >
   >La personne responsable peut clore la tâche à tout moment, même si des validations sont manquantes. Le statut de la progression passe alors automatiquement à 100 %.
...
1. La tâche passe alors au statut **[!UICONTROL Terminé]**. Une notification est envoyée à toutes les personnes concernées.
