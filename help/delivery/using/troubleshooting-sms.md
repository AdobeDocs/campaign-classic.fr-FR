...
* **Reçu sur le mobile** :
Le message a été correctement diffusé sur l’appareil de l’utilisateur, avec une confirmation fournie par la diffusion Mobile Terminated (MT) et un rapport de statut (SR).

* **Envoyé** :
Le message a été traité avec succès par l’étape Mobile Terminated (MT) , mais aucun rapport d’état (SR) confirmant la diffusion sur l’appareil mobile n’a encore été reçu.

* **Envoyé au fournisseur** :
Le message a été envoyé au fournisseur à l’aide de l’`SUBMIT_SM command`, mais aucun accusé de réception `SUBMIT_SM_RESP` n’a été reçu du fournisseur.
