...
* **http://hostname/nl/jsp/schemawsdl.jsp?schema=nms:batchEvent** pour accéder au schéma de type lot.
...
Si votre serveur présente une répartition de la charge, vous pouvez utiliser l’authentification par utilisateur ou utilisatrice/mot de passe (au niveau du message RT).Exemple:
...
Si vous souhaitez différer la diffusion d’un événement, ajoutez le champ **[!UICONTROL planifié]** suivi de la date souhaitée.L’événement sera transformé en message à cette date.

Nous vous recommandons de renseigner les attributs @wishedChannel et @emailFormat avec des valeurs numériques.La table de correspondance entre les valeurs numériques et les libellés associés se trouve dans la description des schémas de données.

>[!NOTE]
>
>Une description détaillée de tous les attributs autorisés, ainsi que de leurs valeurs, est disponible dans la description des schémas de données **nms:rtEvent** et **nms:BatchEvent**.
...
Lorsqu’il reçoit un événement, Adobe Campaign génère un ID de retour unique.Il s’agit de l’ID de la version archivée de l’événement.

>[!IMPORTANT]
>
>Lors de la réception d’appels SOAP, Adobe Campaign vérifie le format de l’adresse e-mail.Si une adresse e-mail n’est pas correctement formatée, une erreur est renvoyée.
