...
1. Accédez au package   à partir de **[!UICONTROL Tools > Advanced > Package import...]** la console client de .
...
1. Installez le que vous souhaitez, par exemple le **mobile**, à l’aide du **[!UICONTROL Tools > Advanced > Import package > Adobe Campaign package]** de l’ensemble  ().
...
1. Dans **[!UICONTROL XML data content to import]**, conservez uniquement le du centre de messages correspondant au ci-joint. Par exemple, si vous avez ajouté le **Mobile**, conservez uniquement l’élément **entités** correspondant au modèle **[!UICONTROL Mobile transactional message]** (smsTriggerMessage). Si vous avez ajouté le **Mobile App**, conservez uniquement les modèles de **** iOS (iosTriggerMessage) et les **modèles d’** Android (androidTriggerMessage).
