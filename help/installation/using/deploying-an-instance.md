...
* **[!UICONTROL Common name of the customer]** : Entrez une chaîne de caractères avec le nom de votre société. Ces informations peuvent être utilisées dans les liens de désinscription.
* **[!UICONTROL Namespace]** : Entrez un identifiant court en minuscules. L&#39;objectif est de faire la distinction entre votre configuration spécifique et la configuration d&#39;usine en cas de mise à niveau. L’espace de nommage par défaut est **cus** - pour le client.
...
* **[!UICONTROL Email channel]** : pour activer la diffusion de messagerie. Voir [Paramètres du canal email](#email-channel-parameters).
...
* **[!UICONTROL Managing bounced emails]** : Pour définir le compte POP utilisé pour récupérer les e-mails entrants. Voir [Gestion des emails rebonds](#managing-bounced-emails).
* **[!UICONTROL LDAP integration]** : Configuration de l’authentification des utilisateurs via un annuaire LDAP Voir [Connexion par LDAP](../../installation/using/connecting-through-ldap.md).
...
* **[!UICONTROL Error address]** : Adresse électronique des messages contenant des erreurs. Il s’agit de l’adresse technique utilisée pour gérer les messages de rebonds, y compris les courriers électroniques reçus par le serveur Adobe Campaign en raison d’adresses de cible inexistantes.
...
* **[!UICONTROL Delivery duration of messages]** : Au-delà de ce délai, la diffusion est arrêtée (par défaut, 5 jours),
* **[!UICONTROL Online resources validity duration]** : Heure à laquelle les informations provenant du profil destinataire sont conservées afin de générer des pages miroir,
* **[!UICONTROL Exclude recipients who no longer wish to be contacted]** : Lorsque cette option est sélectionnée, les destinataires blacklistés ne sont pas contactés,
* **[!UICONTROL Automatically ignore doubles]** : Lorsque cette option est sélectionnée, la diffusion ne sera pas effectuée aux adresses de duplicata.
...
* **[!UICONTROL Duration between two significant errors]** : entrez une valeur (&quot;1d&quot; par défaut : 1 jour) pour définir le délai d’attente de l’application avant d’incrémenter le compteur d’erreurs en cas d’échec,
...
* **[!UICONTROL Label]** : Nom qui comprend tous les paramètres indiqués ci-dessous,
* **[!UICONTROL Server]** : Serveur utilisé pour récupérer le courrier de rebond (courrier entrant),
...
* **[!UICONTROL Account]** : Nom du compte utilisé pour le courrier de rebond,
...
* **[!UICONTROL Forwarding address]** : Renseignez ce champ pour transférer à un tiers l’adresse de tous les messages d’erreur (traités ou non traités) collectés par la plateforme Adobe Campaign.
* **[!UICONTROL Address for errors]** : Renseignez ce champ pour transférer à une adresse tierce uniquement les messages d&#39;erreur que le processus inMail n&#39;a pas été en mesure de qualifier.
* **[!UICONTROL SMTP server]** : Serveur utilisé pour envoyer les courriers électroniques de rebond non traités.

>[!CAUTION]
>
>Pour transférer des courriers électroniques de retour non traités, Adobe recommande de ne renseigner que le **[!UICONTROL Address for errors]** champ. Cependant, assurez-vous que l&#39;adresse utilisée est vérifiée régulièrement, car cela pourrait mettre une charge importante sur votre serveur de messagerie. Contactez le gestionnaire de votre compte pour plus d’informations.
...
* Pour les champs **[!UICONTROL Server]**, **[!UICONTROL Port]**, **[!UICONTROL Account]**, **[!UICONTROL Password]**, **[!UICONTROL Connector]**, **[!UICONTROL Send Endpoint]**, **[!UICONTROL Reception Endpoint]**,  : Contactez votre prestataire pour connaître les paramètres requis.**[!UICONTROL Notification Endpoint]**
...
* **[!UICONTROL Time between two significant errors]** : Saisissez une valeur par défaut (par défaut, &quot;1d&quot;) : day) pour définir l’heure à laquelle l’application attend avant d’incrémenter le compteur d’erreurs en cas d’échec.
* **[!UICONTROL Maximum number of errors before quarantine]** : Une fois cette valeur atteinte, le numéro de mobile est mis en quarantaine (par défaut, &quot;5&quot; : le numéro sera mis en quarantaine à la sixième erreur). Cela signifie que le contact sera automatiquement exclu des diffusions futures.
...
* **[!UICONTROL Consider all phone numbers as international ones]** : Lorsque cette option est sélectionnée, l&#39;application applique le format international aux numéros de téléphone (le préfixe de pays est alors obligatoire car le nombre de chiffres ne sera pas vérifié avant l&#39;application de la mise en forme). Si cette option n’est pas sélectionnée, vous devez préfixer vous-même le numéro de téléphone international avec le signe &quot;+&quot; ou &quot;00&quot;.
* **[!UICONTROL Store all phone numbers using the international format]** : Cette option concerne uniquement les numéros de téléphone **nationaux** qui sont importés ou modifiés. Indiquez si vous souhaitez utiliser un format national (par exemple, 425 555 0150) ou international (par ex. +1 425 555 0150)
...
* Autre(s) serveur(s) Adobe Campaign
   ...
   Les paramètres du ou des serveurs dédiés sont indiqués dans les **[!UICONTROL Media URL(s)]**, **[!UICONTROL Password]** et **[!UICONTROL Instance name]** champs.
