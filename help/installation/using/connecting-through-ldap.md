...
* Sélectionnez dans la liste déroulante le mécanisme d&#39;authentification des utilisateurs :
   ...
   * Mot de passe en texte brut + SSL (**TLS**) : l’ensemble de la procédure d’authentification (mot de passe inclus) est chiffrée. Le port sécurisé 636 ne doit pas être utilisé dans ce mode : Adobe Campaign passe automatiquement en mode sécurisé.
     ...
     Lorsque vous utilisez ce mode d’authentification, sous Linux, le certificat est vérifié par une bibliothèque cliente openLDAP.Nous recommandons d’utiliser un certificat SSL valide afin que la procédure d’authentification soit chiffrée.Sinon, les informations seront en texte brut.
...
Les systèmes compatibles dépendent du mécanisme d’authentification sélectionné.Le tableau suivant est une matrice de compatibilité des systèmes d’exploitation et des serveurs LDAP.
...
* Sélectionnez l&#39;option **[!UICONTROL Activer la synchronisation des droits utilisateurs depuis les autorisations ou groupes de l&#39;annuaire]** afin de synchroniser l&#39;association entre les groupes et les utilisateurs dans l&#39;annuaire LDAP et l&#39;association entre les groupes et les utilisateurs dans Adobe Campaign.
  ...
  Si vous renseignez ces deux champs. Adobe Campaign se connectera au serveur LDAP avec son propre identifiant et mot de passe.S’ils sont vides, Adobe Campaign se connectera anonymement au serveur.
...
* Dans les champs **[!UICONTROL DN applicatif pour la recherche]** et **[!UICONTROL Mot de passe du login applicatif]**, indiquez l’identifiant et le mot de passe avec lesquels Adobe Campaign se connectera pour rechercher l’identifiant.S’ils sont vides, Adobe Campaign se connectera anonymement au serveur.
...
1. Des groupes existent dans Adobe Campaign et l’utilisation de l’annuaire LDAP est activée a posteriori : les groupes d’utilisateurs et d’utilisatrices dans Adobe Campaign ne sont pas remplacés automatiquement par le contenu des groupes LDAP.De même, si un groupe n’existe que dans Adobe Campaign, aucun utilisateur ni aucune utilisatrice LDAP ne peut y être ajouté(e) avant que le groupe ne soit créé et synchronisé dans LDAP.

   Les groupes ne sont jamais créés à la volée, ni par Adobe Campaign, ni par LDAP.Ils doivent être créés individuellement à la fois dans Adobe Campaign et dans l’annuaire LDAP.

   Les noms des groupes dans l’annuaire LDAP doivent correspondre aux noms des groupes dans Adobe Campaign.Leur masque de correspondance est défini dans la dernière étape de configuration de l’assistant de déploiement : Adobe Campaign_(.&#42;), par exemple.
